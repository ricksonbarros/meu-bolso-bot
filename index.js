...
// Garantir que o BOT_TOKEN está definido
if (!process.env.BOT_TOKEN) {
  console.error("❌ BOT_TOKEN não definido! Encerrando...");
  process.exit(1);
}
...
function startBot() {
  ...
  console.log("🤖 Bot financeiro rodando com polling seguro...");
  ...
  bot.on("message", async (msg) => {
    ...
    if (text.startsWith("/saldo")) {
      ...
      bot.sendMessage(chatId, `💰 Seu saldo atual: R$ ${saldo?.toFixed(2) || 0}`);
      return;
    }
...
// Inicia bot com try/catch para evitar conflito
try {
  startBot();
} catch (err) {
